...
class ClusterData ():
    def estimate_haversine(self):
        ...
        weight_eucl =1./self._distance_matr.max()
        weight_hav =1./angdist_matr.max()
    ...
    def estimate_affinity (self, affinity , file_affinity ):
        ...
        if affinity == 'euclidean':
            ...
            assert self._nfeat > 1, ( f"can't estimate hellinger distance with {self._nfeat} \
            ...
            raise ValueError(f" Affinity  '{affinity}' not recognized,"
                             "  please use either  'euclidean' or 'hellinger' " )

        self._distance_matr = self._metric.pairwise (self._X)
        self._distance_matr=pl.ma.fix_invalid(self._distance_matr,  fill_value=1.).data
    ...
    def intracluster_distance(self, K, labels  ):
        ...
        for k in range(K):
            ...
            E  = ( self._metric.pairwise( X = Xk[:, :self._nfeat],
                         Y=  mu[k,:self._nfeat].reshape(-1, self._nfeat) ) )

            E = pl.ma.fix_invalid(E,  fill_value=1.).data
            if self._has_angles and Nk >1 :
                H = self.haversine.pairwise (X = Xk[:, self._nfeat:],
                                            Y=  mu[k,self._nfeat:].reshape(-1, 2) )
    ...
    def minimize_partition_measures(self  ):
        self.Vu=pl.zeros_like(self.Kvals).reshape(-1,1)*1. ; self.Vo= pl.zeros_like(self.Kvals).reshape(-1,1)*1.
        ...
        for j, K  in enumerate( self.Kvals )  :
            ...
            dmu = pl.ma.fix_invalid(dmu,  fill_value=1.).data
            if self._has_angles:
    ...
    def estimate_Gap_statistics (self,nrefs):
        ...
        for i in range(nrefs):
            ...
            random_X [:,1 ] =np.random.uniform(low = pl.quantile(q= 0.16, a =self._X[masknans,1]), high=pl.quantile(q= 0.16, a =self._X[masknans,1]),  size=pl.int_( self._X.shape[0] ) )
            random_X [:,0 ] =np.random.normal (loc = meanvals[0]   , scale=stdvals[0],  size=pl.int_( self._X.shape[0]  ) )
            ref_D = self._metric.pairwise (random_X)
            ref_D=pl.ma.fix_invalid(ref_D,  fill_value=1.).data
    ...
    def maximize_Krzanowski_Lai_index (self):
        ...
        DIFF = pl.array ( [ self.Kvals[1:] ,
                   (self.W [:-1]  *self.Kvals[:-1]**(2/p) )-(self.W [1: ] *self.Kvals[1: ]**(2/p)  )  ] )

        #for k=1, KL index is undefined

        self.KL =pl.array ( [ self.Kvals[1:-1] ,
    ...
    def __call__ (self, K=None , nvals=10, Kmax= 50,Kmin=2, minimize = 'partition' , **kwargs ):
        ...
        if self.verbose  : print("Running Hierarchical clustering "
