...
from fgcluster.utils import from_ell_to_index,from_index_to_ell
from fgcluster.spectral_clustering_mpi import ( build_adjacency_from_heat_kernel_gather,
                                    build_adjacency_from_nearest_neighbours,
                                    build_adjacency_from_KS_distance_gather,
                                    build_adjacency_from_KS_distance_nearest_neighbours,
...
def test_spectral_clustering( comm,workdir ):
    ...
    s =time.perf_counter ()
    A = build_adjacency_from_heat_kernel_gather (nside, comm  )
    e = time.perf_counter ()
    if rank==0 :
...
def test_KS_distances(comm,workdir ):
    ...
    Q   = build_adjacency_from_KS_distance_gather(  nside=nside , comm=comm,
                                    X= X, sigmaX=sigmaX  )
    e = time.perf_counter ()
    if rank==0:
    ...
    A   = build_adjacency_from_heat_kernel_gather(  nside=nside , comm=comm, KS_weighted=True ,
                                                    Q=Q  ,alpha=.5)
    e = time.perf_counter ()
    ...
    Q   = build_adjacency_from_KS_distance_nearest_neighbours(  nside=nside , comm=comm,
                                    X= X, sigmaX=sigmaX , order_nn=0 )
    e = time.perf_counter ()
    if rank==0:
        #pl.save(f'{workdir}/test/adjacency_KS_nn.npy', Q)
        assert np.allclose(pl.load(f'{workdir}/test/adjacency_KS_nn.npy'), Q, atol=1e-5)
        print(f"build_adjacency_from_KS_distance_nearest_neighbours, execution time {e-s }sec")
