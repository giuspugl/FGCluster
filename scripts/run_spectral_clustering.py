...
from fgcluster.spectral_clustering_mpi import (
    build_adjacency_from_heat_kernel,
...
def main(args):
    ...
    sigmaparam[mask] = param[mask] * 0.0005
    if rank == 0:
    ...
    if np.bool(args.KS_weight):
        if path.exists(f"{workdir}/affinities/KS_distance_{string}_{args.nside}.npz"):
            Q = np.load(f"{workdir}/affinities/KS_distance_{string}_{args.nside}.npz")[
                "affinity"
            ]
        else:
            Q = build_adjacency_from_KS_distance(
                nside=nside,
                comm=comm,
                X=param,
                sigmaX=sigmaparam,
    ...
    A = build_adjacency_from_heat_kernel(
        nside, comm, KS_weighted=np.bool(args.KS_weight), Q=Q, alpha=args.KS_weight
    )
    if rank == 0:
        pl.subplot(121)
