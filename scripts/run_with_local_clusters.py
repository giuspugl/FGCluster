...
from fgcluster import utils as cu
...
def main(args):
    ...
    firstpix, lastpix = cu.split_data_among_processors(
        size=hp.nside2npix(nside_superpixel), rank=rank, nprocs=nprocs
    )
    ...
    for ipixloc in np.arange(firstpix, lastpix):
        ...
        if np.bool_(args.KS_weight):
            Q = build_adjacency_from_wasserstein_distance(
                X=X, sigmaX=sigmaX, nresample=256, nsigma=2
            )
        else:
            Q = None

        A = build_adjacency_from_heat_kernel_local(
            pixs=listp,
            nside=hp.get_nside(param),
        ...
        l, W = estimate_Ritz_eigenpairs(
            L, n_eig=np.int_(cu.from_index_to_ell(A.shape[1]))
        )
        ...
        order_ncl = np.int_(np.ceil(np.log10(clusters.n_clusters_)))
        tmp = ipixloc * 10**order_ncl + clusters.labels_
        mapout_labels = cu.mappify(arr=tmp, nside=hp.get_nside(param), pixs=listp)
    ...
    repix = np.unique(patches )
    newpatches= np.zeros_like(patches)
    print(repix.size )
    for i, pix in enumerate(repix):
        idx = patches == pix
        newpatches[idx]= i

    if rank == 0:
        hp.mollview(newpatches )
        pl.show()
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="prepare training and testing dataset from a healpix map "
    )

    parser.add_argument(
        "--spectral-parameter",
        help="spectral  parameter (Bd, Td, Bs) to perform clustering",
    )
    parser.add_argument(
        "--parameter-uncertainties",
        help="path to the  healpix  map of uncertainties",
    )

    parser.add_argument(
        "--nside", help="nside of output maps", required=True, type=np.int_
    )
    parser.add_argument(
        "--KS-weight",
        help="weight to the KS distance to be combined with heat kernel",
        required=True,
        type=np.float_,
    )
    parser.add_argument(
        "--distance-threshold",
        help="threshold to define connectivity in AgglomerativeClustering",
        default=0.3,
        type=np.float_,
    )
