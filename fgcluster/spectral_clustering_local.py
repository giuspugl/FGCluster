...
def build_adjacency_from_heat_kernel_local(
    ...
    **Parameters**
    ...
    for l in np.arange(lmax):
        Gloc += fgc.heat_kernel(scalprod, l, sigmabeam)
    return Gloc


def build_adjacency_from_wasserstein_distance(X, sigmaX, nresample=256, nsigma=2):
    """
    ...
    for i in range(npix - 1):
        v2 = np.random.normal(
            loc=X[i] + nsigma * sigmaX[i], scale=nsigma * sigmaX[i], size=nresample
        )
...
def estimate_Laplacian_matrix_sparse(W, kind="unnormalized"):
    ...
    if kind == "unnormalized":
        ...
        Dinv2s = sparse.spdiags(
            1 / np.sqrt(W.diagonal()), diags=0, m=W.shape[0], n=W.shape[1]
        )
